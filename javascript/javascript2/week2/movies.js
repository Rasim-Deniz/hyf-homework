...
// 1
const shortTitled = movies.filter((movie) => movie.title.length <= 5);
// 2
const longTitled = movies.filter((movie) => movie.title.length >= 10);
// 3
const betweenYears = movies.filter(
  (movie) => movie.year >= 1980 && movie.year <= 1989
).length;
// 4
const taggedMovies = movies.filter((movie) => {
  let tag = movie.rating;
  if (tag >= 7) {
...
// 5
const ratedCount = movies
  .filter((movie) => movie.rating > 6)
  .map((movie) => movie.rating);
// 6
const keywordCount = movies.filter(
  (movie) =>
...
// 7
const duplicateTitled = movies.filter((movie) => {
  let titleArray = movie.title.toLowerCase().split(" ");
  let findDuplicates = titleArray.filter(
    (item, index) => titleArray.indexOf(item) != index
  );
  return findDuplicates.length;
});
// 8
const averageRating =
  movies.reduce((average, movie) => average + movie.rating, 0) / movies.length;
// 9
const totalRated = taggedMovies.reduce(
  (total, movie) => {
    let rate = movie.tag;
    if (rate === "Good") {
      total.goodMovies++;
...
console.log(shortTitled);
console.log(longTitled);
console.log(betweenYears);
console.log(taggedMovies);
console.log(ratedCount);
console.log(keywordCount);
console.log(duplicateTitled);
